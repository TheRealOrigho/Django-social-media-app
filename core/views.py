...
def follow(request):
    ...
    if request.method == "POST":
        ...
        if FollowersCount.objects.filter(follower=follower, user=user).first():
            ...
            return redirect('/profile/'+user) #HttpResponseRedirect(request.META.get('HTTP_REFERER'))
    ...
    if request.method == 'POST':
        ...
        if password == password2:
            if User.objects.filter(email=email).exists():
                ...
                #login user and redirect to setings
...
def signin(request):
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            messages.info(request, 'User Credential Invalid')
